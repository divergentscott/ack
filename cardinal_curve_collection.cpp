...
bool CardinalCurveCollection::rayTraceNorth(const Eigen::Vector2d &origin,
	Eigen::Vector2d &impact,
	int& impact_edge_id) {
	bool is_impact = false;
	double closest_y_yet = std::numeric_limits<double>::max();
	for (auto e_foo = 0; e_foo < edges_.size(); e_foo++) {
		if (is_horizontals_[e_foo]) {
			Eigen::Vector2d p0 = points_[edges_[e_foo][0]];
			if ((p0[1] < closest_y_yet) & (p0[1] > origin[1])) {
				Eigen::Vector2d p1 = points_[edges_[e_foo][1]];
				if (p1[0] < p0[0]) std::swap(p0, p1);
				Eigen::Vector2d impact_foo;
				Vedge segment = { p0,p1 };
				bool is_foo_hit = rayNorthSegmentIntersect(origin, segment, impact_foo);
				if (is_foo_hit) {
					is_impact = true;
					impact = impact_foo;
					impact_edge_id = e_foo;
					closest_y_yet = impact_foo[1];
				}
			}
		}
	}
